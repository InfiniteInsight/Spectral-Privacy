	...
	{#if open}
		...
		>
			...
			<div class="border-t border-gray-100 pt-1">
				<a href="/people" class="block px-4 py-2 text-sm text-gray-500 hover:bg-gray-50"
					>Manage vaults…</a
				>
			</div>
		</div>
		...
		>
			...
			<div class="flex gap-3">
				<button
					...
					style="background-color: {unlockLoading || !unlockPassword
						? '#d1d5db'
						: '#0284c7'}; color: white;">{unlockLoading ? 'Unlocking...' : 'Unlock'}</button
				>
				<button
					onclick={() => {
						unlockModalVaultId = null;
